...
(function (ag) {
    ko.bindingHandlers["filter"] = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            var $element = $(element), source = valueAccessor(), net = bindingContext.$root.net || new ag.utils.Network(), $data = bindingContext.$data;
            ...
            if (prerequisiteFields != undefined) {
                var fields = prerequisiteFields.split(","), fieldsString = "";
                ...
                if (fieldsString.length > 0)
                    element.title = ag.strings.prereqFields.format(fieldsString);
            }
            ...
            var onAfterSave = function () {
                ...
                ag.key.utils.notifyKeyEventChange($element);
            };
            ...
            if ($element.is("input")) {
                ...
                ag.filter.helper.typeahead.init($element, maxItems, dataSource, optionsTitle, optionsText, compareKey, optionsValue, valueCallback, hideQueryUI, hintSource, hintTarget, prefix, $data, restrictToList);
                ...
                if (ko.isObservable(valueCallback) && restrictToList) {
                    ...
                    ag.utils.registerBindingRule(element, valueCallback, {
                        ...
                        validator: function (value, params, resultCallback) {
                            return validateValueIsInList(value, valueCallback, dataSource, maxItems, optionsValue, resultCallback);
                        }
                    });
                }
            }
            ...
            $element.data("dependencyHandler", {
                setAvailability: function (value) {
    ...
    function createDataSource(selectedKeys, target, requestDataCallback, additionalPayloadData, net, source, postRequest, postFilter) {
        return function (query, maxItems) {
            // Initial lookup data
            var lookupData = {
                pageSize: maxItems,
            ...
            return net.postJson({ url: source }, ag.utils.cleanJSForRequest(lookupData)).then(function (result) {
                ...
                if (postFilter)
                    lookupData.data = $.grep(lookupData.data, function (item) {
    ...
    function validateValueIsInList(value, valueCallback, dataSource, maxItems, optionsValue, resultCallback) {
        if (ag.isNullUndefinedOrEmpty(value) || !valueCallback.valueIsUnvalidated) {
            ...
            dataSource(value, maxItems).done(function (result) {
