...
module ag 
{
   ...
   ko.bindingHandlers["collection"] =
   {
      ...
      {
         var list = $(element),
            selectedItems = valueAccessor(),
            allBindings = allBindingsAccessor(),
            optionsTitle = allBindings.optionsTitle,
            optionsText = allBindings.optionsText,
            beforeRemoveCallback = allBindings.beforeRemove,
            bindKeyColumn = !!list.data("lookup-bind-key-column"),
            keyField = list.data("lookup-key-field");
         ...
         list.on("click", "a.icon-remove, .close.action", event =>
         {
            var currentSelectedItems = valueAccessor(),
               observable = ko.dataFor(event.target);
      ...
      update: (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) =>
      {
         var list = $(element),
            ...
            optionsTitle = allBindingsAccessor().optionsTitle,
            bindKeyColumn = !!list.data("lookup-bind-key-column"),
            keyField = list.data("lookup-key-field");
