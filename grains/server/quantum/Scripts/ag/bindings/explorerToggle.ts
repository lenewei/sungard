...
module ag
{
   ...
   ko.bindingHandlers["explorerToggle"] =
   {
      ...
      {
         var target = $(element),
            searchInput = $($(target.parent().find('input'))[0]),
            opts = valueAccessor(),
            ...
            initialRequestDataCallback = opts.initialRequestDataCallback,
            ...
            originalSaveCallback = allBindings.saveCallback,
            saveCallback = originalSaveCallback,
            optionsText = allBindings.optionsText,
            optionsTitle = allBindings.optionsTitle,
            beforeClick = allBindings.beforeClick,
            lookupDisplayName = target.data("lookup-display-name"),
            clickBubble = allBindings.clickBubble !== undefined ? ko.unwrap(allBindings.clickBubble) : true,
            cacheOnClient = !!target.data("lookup-cache-on-client"),
            updateFields = target.data("lookup-update-fields"),
            lookupFields = target.data("lookup-lookup-fields"),
            includeCompleteModel = target.data("lookup-include-complete-model");
         ...
         if (mode == "single" && updateFields)
         {
            ...
            saveCallback = lookupItems =>
            {
               ...
               $.each(updateFields, (ind, fieldPath) =>
               {
               ...
               key.utils.notifyKeyEventChange(target.siblings('input'));
            };
         }
         ...
         var toggleExplorer = e =>
         {
            // Reset the search
            (<any>initOptions).searchQuery = "";
         ...
         if (searchInput)
         {
            var debouncedSearch = _.debounce(() =>
            {
            ...
            ko.utils.registerEventHandler(searchInput, "keypress", e =>
            {
               switch (e.keyCode)
               {
                  case (<any>$).ui.keyCode.TAB:
                     break;
                  case (<any>$).ui.keyCode.ENTER:
                     explorer.toggle(initOptions);
