...
module ag.help
{
   ...
   var initialised = false,
      active = false,
      currentUrl = "",
      urlTemplate = "help/content?area={0}&page={1}&field={2}",
      debouncedHelpRequest = _.debounce(updateHelpPanelContentForElement, 600);

   function updateHelpPanelContentForElement(element: JQuery)
   {
   ...
   {
      var ancestors = target.parents(".panel[id]"),
         id = sanitizeId(getId(target));
   ...
   function getHelpContentRequest(helpUrl)
   {
      ...
      {
         url: helpUrl,
         cache: true, /* globally caching is disabled for ajax but we want it here */
         success: (result) =>
         {
   ...
   function toggleHelp(url: string)
   {
      ...
      if (active)
      {
         ...
         dom.setFullWidthCss(true, false);
         getHelpContentRequest(url);
      }
      else
      {
         // set the page width back to its persisted size
         dom.setFullWidthCss(dom.isFullWidth(), false);
      }
   }
   ...
   function init()
   {
      ...
      if (utils.getQueryStringParameterByName("helpmode"))
   ...
   function initHelpActivationHandlers()
   {
      ...
      $("a[data-toggle=showHelpPanel]").click((event) =>
      {
   ...
   function initHelp()
   {
      $(":input, .header>h1").on("focus click", (event) =>
      {
         if (active)
   ...
   {      
      $(":input, .heading, a:not(.app-link)").mouseover((event) =>
      {
...
module ag.tours
{
   function setupTour(data) : any
   {
      var tour = new Tour();
      $.each(data.Steps, (index, step) =>
      {
         tour.addStep({
   ...
   function getTour(name, restart)
   {
      ...
      ag.utils.getJson({ url: url }, { name: name }).then((result) =>
      {
