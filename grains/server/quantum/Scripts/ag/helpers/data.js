...
(function (ag) {
    ...
    var process = function (lookupDataResponse, options) {
        if (!lookupDataResponse)
            return null;
        var dataRows = lookupDataResponse.data;

        var totalItems = ko.unwrap(dataRows).length, start = 0, end = totalItems;
    ...
    var getData = function (source, options, success, error) {
        ...
        if (typeof unwrappedSource == "string") {
            ...
            if (cachedData) {
                ...
                net.postJson({ url: unwrappedSource }, requestData).then(function (response) {
                    ...
                    if (options.cacheOnClient)
                        response = process(response, response.gridViewOptions);
    ...
    (function (data) {
        ...
        data.get = function (source, options, cb) {
            var error;
