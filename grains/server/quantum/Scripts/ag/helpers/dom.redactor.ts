module ag.dom.redactor
{
   export function init($element: JQuery)
   {
      $element.redactor(
         {
            minHeight: 200,
      ...
      if (!redactor.linkProcess || !redactor.linkShow)
         throw new Error(strings.badResponse);
      else
      {
   ...
   export function redactorInit()
   {
      ko.utils.registerEventHandler(document, "click",(event: JQueryEventObject) =>
      {
   ...
   // Insert Html
   export function insertHtml($element: JQuery, value): void
   {
      $element.redactor("set", value);
   }

   // Get Html
   export function getHTML($element: JQuery): string
   {
      return $element.redactor("get");
   }

   export function fixLinks($element: JQuery)
   {
      var processor = (html: HTMLElement) =>
      {
         var $html = $(html),
            content = $html.text();
   ...
   function fixBrokenLinkElement($link: JQuery, content: string)
   {
      var reg = /".+?"/igm;
   ...
   export function displayLocalFileLinkMessage(href: string): void
   {
      var zindex = ag.utils.findHighestDivZIndex();

      var template =
         '<div class="modal-scrollable" style="z-index: {0}; background-color: black; opacity: 0.2"></div>' +
         '<div class="modal-scrollable xclose" style="z-index: {0}">' +
         '   <div class="modal" data-backdrop="static" aria-hidden="false" tabindex="-1" style="display: block; margin-top: -112px;">' +
         '      <form onsubmit="return false;">' +
         '         <div class="modal-header">' +
         '            <div class="row-fluid">' +
         '               <h3 class="span8">{2}</h3>' +
         '               <button type="button" class="close xclose" data-dismiss="modal" aria-hidden="true">×</button>' +
         '            </div>' +
         '         </div>' +
         '         <div class="modal-body container-panel">' +
         '            <div class="panel" data-title="">' +
         '            <span class="editor field size6">{4}</span><p><p/>' +
         '               <div class="editor field size6">' +
         '                  <label>{3}</label>' +
         '                  <input class="text-box single-line valid" type="text" value="{1}" data-original-title="">' +
         '               </div>' +
         '            </div>' +
         '         </div>' +
         '         <div class="modal-footer xclose"><a href="#" class="btn xclose">Close</a></div>' +
         '      </form>' +
         '   </div>' +
         '</div>';

      template = template.format(zindex, href, strings.redactorTitle, strings.redactorFilePath, strings.redactorMessage);
      ...
      ko.utils.registerEventHandler($template, "click",(event: JQueryEventObject) =>
      {
         if ($(event.target).hasClass("xclose"))
            removeDelegate();
      });

      ko.utils.registerEventHandler($template, "keydown",(event: JQueryEventObject) =>
      {
