...
module ag.dates
{
   ...
   export var riskRelativeDateLabels = {
      data: [
         "Today",
         "Yesterday",
         "Tomorrow",
   ...
   export var riskColumnSetRelativeDateLabels = {
      data: [
         "Query Position Date",
         "Query As At Date",
         "Query Start Date",
   ...
   var separatorReg = /[.\/\\\s\-]/g,
      keywordFunctions = [(d) => d, tomorrow, yesterday, endOfMonth, endOfLastMonth, endOfYear, endOfWeek, endOfLastWeek, startOfMonth, startOfLastMonth, theBeginningDate, theEndDate];
   ...
   {
      return userLocationToday.toEditor();
   }
   ...
   export function isDateISO8601Format(value): boolean
   {
   ...
   export function looselyConvertStringToIso(value: string): string
   {
   ...
   export function getLooseDateFormat(value: string): string
   {
   ...
   export function getCleanShortDateFormat()
   {
      return dateShortFormat.replace(separatorReg, "").toUpperCase();
   }

   export function isYearFirstFormat()
   {
      return dateShortFormat.startsWith("Y");
   }

   export function getDateFormatParts(): string[]
   {
      return dateShortFormat.split(separatorReg);
   }
   ...
   export function sanitizeInputString(value: string): string
   {
      ...
      else
      {
         ...
         var parts = value.split(separatorReg),
            partsLength = parts.length,
            indexOfYear = getDateFormatParts().indexOf("YYYY");
   ...
   export function formatDatePart(datePart: string, isYear: boolean): string
   {
      // If we have an null, empty, or "0" datePart return immediately
      if (isNullUndefinedOrEmpty(datePart) || datePart == "0")
         return "";
   ...
   export function resolveDate(inputValue)
   {
      if (!inputValue || _.isDate(inputValue))
         return inputValue;

      var newValue = null,
         date = moment.fromISO(userLocationToday.toISO()).toDate(), // Need to copy userLocationToday to avoid overwriting
         keywordIndex = null,
         keywordLength = null,
         keywords = ["Today", "Tomorrow", "Yesterday", "EndOfMonth", "EndOfLastMonth", "EndOfYear", "EndOfWeek", "EndOfLastWeek", "StartOfMonth", "StartOfLastMonth", "TheBeginning", "TheEnd"];
      ...
      if (!matches)
      {
         ...
         $.each(keywords, i =>
         {
            ...
            if (searchTerm.match(rKeywords))
            {
               ...
               keywordIndex = i;
               return false; // Immediately break the loop
            }
         });

         if (matches) // Matches one of the keywords "Today","Tomorrow"...
         {
            newValue = date;
         ...
         if (!matches)
         {
            ...
            keywordLength = dateShortFormat.length; // ie length of DD/MM/YYYY
            ...
            if (matches)
            {
               var dateString = matches[1],
                  matchesDate = dateString.match(/([0-9]{2,4})[- /.]([0-9]{2})[- /.]([0-9]{2,4})/),
                  sanitized = sanitizeInputString(matchesDate[0]);
      ...
      if (matches)
      {
         var str = matches[0].substr(matches[0].length - 1, 1),
            num = Math.abs(matches[0].substr(keywordLength + 1, (matches[0].length - 2 - keywordLength)));
