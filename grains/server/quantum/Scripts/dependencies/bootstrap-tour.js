   ...
   (function ($, window)
   {
      ...
      Tour = (function ()
      {
         ...
         function Tour(options)
         {
            ...
            this.setCurrentStep();
            $(document)
               .off("click", ".popover .next")
               .on("click", ".popover .next", function (e)
               {
                  e.preventDefault();
                  return _this.next();
               });
            $(document)
               .off("click", ".popover .prev")
               .on("click", ".popover .prev", function (e)
               {
                  e.preventDefault();
                  return _this.prev();
               });
            $(document)
               .off("click", ".popover .end")
               .on("click", ".popover .end", function (e)
               {
                  e.preventDefault();
                  return _this.end();
               });
         ...
         Tour.prototype.start = function (force)
         {
            ...
            if (force || !this.ended() && this.navigated())
            {
               this.setState("navigated", null);
         ...
         Tour.prototype.navigated = function ()
         {
            return !!this.getState("navigated");
         };
         ...
         Tour.prototype.showStep = function (i)
         {
            ...
            if (step.path !== "" && document.location.pathname.replace(/\/?$/, '') !== step.path.replace(/\/?$/, ''))
            {
               this.setState("navigated", "yes");
            ...
            {
               log("skipping step because selector has no matching elements: " + step.element);
               this.showNextStep();
               return;
            }
         ...
         {
            var content, nav, tip;
            content = "<button class='close top-right end'>&times;</button>";
            content += "<p>" + step.content + "</p>";
            nav = [];
            if (step.prev >= 0)
            {
               nav.push("<a href='#" + step.prev + "' class='prev btn'>" + this._options.previous + "</a>");
            }
            if (step.next >= 0)
            {
               nav.push("<a href='#" + step.next + "' class='next btn btn-info'>" + this._options.next + "</a>");
            }
            else
            {
               nav.push("<a href='#' class='end btn btn-success'>Done</a>");
            }
            content += "<div class='footer'>" + nav.join(" ") + "</div>";
